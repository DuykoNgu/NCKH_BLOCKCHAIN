export function TrongDongWatermark({ className = '', opacity = 0.05 }: { className?: string; opacity?: number }) {
  ...
  return (
    <div className={`absolute inset-0 overflow-hidden pointer-events-none ${className}`}>
      <svg
        viewBox="0 0 400 400"
        className="absolute w-[800px] h-[800px] -right-64 -top-64 text-foreground animate-spin-slow"
      ...
      >
        ...
        {Array.from({ length: 12 }).map((_, i) => (
          <g key={`bird-${i}`} transform={`rotate(${i * 30} 200 200)`}>
            <path d="M 200 80 Q 210 95 200 110 Q 190 95 200 80" fill="currentColor" opacity="0.7" />
      ...
      {/* Second smaller watermark bottom-left for depth */}
      <svg
        viewBox="0 0 400 400"
        className="absolute w-[400px] h-[400px] -left-32 -bottom-32 text-foreground animate-spin-slow"
        style={{ opacity: opacity * 0.6, transformOrigin: 'center', animationDirection: 'reverse', animationDuration: '180s' }}
      >
        <circle cx="200" cy="200" r="195" fill="none" stroke="currentColor" strokeWidth="2" />
        <circle cx="200" cy="200" r="165" fill="none" stroke="currentColor" strokeWidth="1" />
        <circle cx="200" cy="200" r="130" fill="none" stroke="currentColor" strokeWidth="1" />
        ...
        <circle cx="200" cy="200" r="8" fill="currentColor" opacity="0.5" />
        {Array.from({ length: 12 }).map((_, i) => (
          <g key={`bird2-${i}`} transform={`rotate(${i * 30} 200 200)`}>
            <path d="M 200 80 Q 210 95 200 110 Q 190 95 200 80" fill="currentColor" opacity="0.7" />
          </g>
        ))}
      </svg>
...
export function TrongDongDivider({ className = '' }: { className?: string }) {
  return (
    <div className={`w-full flex items-center justify-center py-4 ${className}`}>
      <svg viewBox="0 0 800 60" className="w-full max-w-sm h-8 text-foreground/50">
