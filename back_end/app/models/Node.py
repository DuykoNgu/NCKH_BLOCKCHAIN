...
class Node:
    ...
    def __init__(self, node_role: NodeRole, private_key_hex: str = None):
        ...
        if private_key_hex:
            self.pubkey = KeyUtils.get_public_key_from_private(private_key_hex)
        else:
            self.pubkey, self.validator_private_key = KeyUtils.generate_key_pair()
        
        # Tạo address từ public key
        self.address = KeyUtils.get_address_from_pubkey(self.pubkey)
